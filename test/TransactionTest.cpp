...
TEST(TransactionTest, Constructor_InvalidDateFormat) {
    EXPECT_THROW(Transaction("25-04-2024", "Stipendio", 1000.0, in), invalid_argument);
}
...
TEST(TransactionTest, Constructor_EmptyDate) {
    EXPECT_THROW(Transaction("", "stipendio", 100.0, in), invalid_argument);
}


// Test costruttore con descrizione vuota
TEST(TransactionTest, Constructor_EmptyDescription) {
    EXPECT_THROW(Transaction("2024-04-25", "", 1000.0, in), invalid_argument);
}

// Test costruttore con ammonto negativo
TEST(TransactionTest, Constructor_NegativeAmount) {
    EXPECT_THROW(Transaction("2024-04-25", "Stipendio", -1000.0, in), invalid_argument);
}

// Test costruttore con ammonto vuoto
TEST(TransactionTest, Constructor_ZeroAmount) {
    EXPECT_THROW(Transaction("2024-04-25", "Stipendio", 0.0, in), invalid_argument);
}
...
TEST(TransactionTest, SetDescription_Empty) {
    Transaction t("2024-04-25", "Stipendio", 1500.0, in);
    EXPECT_THROW(t.setDescription(""), invalid_argument);
}
...
TEST(TransactionTest, SetAmount_Negative) {
    Transaction t("2024-04-25", "Stipendio", 1500.0, in);
    EXPECT_THROW(t.setAmount(-500.0), invalid_argument);
}
...
TEST(TransactionTest, StringToType_Valid) {
    EXPECT_EQ(Transaction::stringToType("in"), in);
    EXPECT_EQ(Transaction::stringToType("out"), out);
}

// Test stringToType invalido
TEST(TransactionTest, StringToType_Invalid) {
    EXPECT_THROW(Transaction::stringToType("42"), invalid_argument);
}
...
TEST(TransactionTest, FromString_Valid) {
    Transaction t = Transaction::fromString("2024-04-25;Stipendio;1500;in");
    EXPECT_EQ(t.getDate(), "2024-04-25");
    EXPECT_EQ(t.getDescription(), "Stipendio");
    EXPECT_DOUBLE_EQ(t.getAmount(), 1500.0);
    EXPECT_EQ(t.getType(), in);
...
TEST(TransactionTest, FromString_Invalid) {
    EXPECT_THROW(Transaction::fromString("Stringa non valida"), invalid_argument);
}